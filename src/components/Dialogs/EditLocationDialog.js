...
export default function EditLocationDialog({id,handleClose}) {
...
const handleSave=()=>{
   // let out =store.getLocationByName(formData.name);
    ...
    if(formData.name === ""){
        ...
        store.updateLocations(formData);
        handleClose();
    }
...
const setValues=(locid)=>{
   // console.log("id",locid);
    //lets get the values from store then set default values of the form
    let dfv = store.getLocationByID(locid);
   // console.log(dfv[0])
    if(dfv.length > 0){
        handleChangeLocation(dfv[0].coordinates.lat,dfv[0].coordinates.lng)
       setFormData({
           ...formData,...dfv[0]
       })
    }
